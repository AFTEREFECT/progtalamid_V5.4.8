...
interface StudentScheduleAlternateProps {
  section: string;
  studentName?: string;
}
...
export const StudentScheduleAlternate: React.FC<StudentScheduleAlternateProps> = ({ section, studentName }) => {
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
      <div className="bg-gradient-to-r from-green-600 to-teal-600 p-4 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold">جدول الحصص (النسخة الأفقية)</h2>
            {studentName && <p className="text-green-100 text-sm">{studentName}</p>}
            <p className="text-green-100 text-sm">القسم: {section}</p>
          </div>
        </div>
      </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-100">
              <th className="border border-gray-300 px-3 py-2 text-center font-bold">اليوم</th>
              {periods.map(period => (
                <th key={period.label} className="border border-gray-300 px-3 py-2 text-center font-bold text-sm">
          ...
          <tbody>
            {days.map(day => (
              <tr key={day}>
                <td className="border border-gray-300 px-3 py-3 bg-gray-50 font-bold text-center">
                  {day}
                </td>
