...
const ComprehensiveImport: React.FC = () => {
  ...
  // حفظ إعدادات المؤسسة مع حفظ مزدوج قوي
  const saveInstitutionSettings = async (metadata: any) => {
    console.log('💾 [ComprehensiveImport] بدء حفظ إعدادات المؤسسة:', metadata);
    ...
    try {
      await dbManager.saveInstitutionSettings(settingsData);
      dbSuccess = true;
      console.log('✅ [ComprehensiveImport] تم حفظ الإعدادات في قاعدة البيانات');
    } catch (dbError) {
      console.error('❌ [ComprehensiveImport] فشل حفظ الإعدادات في قاعدة البيانات:', dbError);
    }

    // الحفظ في localStorage كاحتياطي
    try {
      localStorage.setItem('institution_settings_backup', JSON.stringify(settingsData));
      localStorage.setItem('institution_settings_timestamp', new Date().toISOString());
      localSuccess = true;
      console.log('✅ [ComprehensiveImport] تم حفظ الإعدادات في localStorage');
    } catch (localError) {
      console.error('❌ [ComprehensiveImport] فشل حفظ الإعدادات في localStorage:', localError);
    }

    if (!dbSuccess && !localSuccess) {
      console.error('❌ [ComprehensiveImport] فشل الحفظ في كل من قاعدة البيانات والتخزين المحلي!');
      throw new Error('فشل حفظ إعدادات المؤسسة بالكامل');
    } else if (dbSuccess && localSuccess) {
      console.log('✅ [ComprehensiveImport] تم حفظ الإعدادات بنجاح في المصدرين');
    } else {
      console.warn('⚠️ [ComprehensiveImport] تم الحفظ جزئياً:', { dbSuccess, localSuccess });
    }
  };
  ...
  const handleComprehensiveImport = async () => {
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          for (const sheetName of workbook.SheetNames) {
            ...
            const range = XLSX.utils.decode_range(worksheet['!ref'] || 'A1:Z1000');
            const totalRows = range.e.r - 10 + 1; // عدد الصفوف من 11 حتى النهاية
            console.log(`📊 ${sheetName}: إجمالي الصفوف للمعالجة = ${totalRows} (من الصف 11 إلى ${range.e.r + 1})`);

            for (let rowIndex = 10; rowIndex <= range.e.r; rowIndex++) {
              try {
                // قراءة الرقم الترتيبي من العمود A - مهم جداً!
                const serialNumberRaw = getCellValue(worksheet, `A${rowIndex + 1}`);
                const serialNumber = serialNumberRaw ? String(serialNumberRaw).trim() : '';
                ...
                // ✅ تجاوز الصفوف الفارغة تماماً فقط
                if (!serialNumber && !nationalId && !lastName && !firstName) {
                  continue;
                }

                // ✅ التحقق من الحقول الأساسية فقط (الاسم واللقب)
                if (!lastName || !firstName) {
                  errorCount++;
                  allErrors.push(`❌ بيانات ناقصة (الاسم أو اللقب) في ${file.name} - ${sheetName} - صف ${rowIndex + 1}: "${firstName} ${lastName}"`);
                  continue;
                }

                // ✅ التحقق من التكرار (فقط إذا كان رمز المسار موجوداً)
                if (nationalId) {
                  if (processedNationalIds.has(nationalId)) {
                    allErrors.push(`⚠️ تكرار: ${nationalId} (${firstName} ${lastName}) في ${file.name} - ${sheetName}`);
                    continue;
                  }
                  processedNationalIds.add(nationalId);
                } else {
                  console.warn(`⚠️ تلميذ بدون رمز مسار: ${firstName} ${lastName} - صف ${rowIndex + 1}`);
                }

                // التأكد من وجود الرقم الترتيبي
                const finalStudentId = serialNumber && serialNumber !== '' && serialNumber !== 'NaN'
                  ? serialNumber
                  : String(rowIndex - 9); // احتياطي فقط

                const student: Student = {
                  id: crypto.randomUUID(),
                  firstName: firstName,
                  lastName: lastName,
                  nationalId: nationalId || '', // ✅ قيمة افتراضية إذا كان فارغاً
                  gender: (gender || 'ذكر') as 'ذكر' | 'أنثى',
                  birthPlace: birthPlace || '', // ✅ قيمة افتراضية
                ...
                // ✅ التحقق من وجود التلميذ مسبقاً (فقط إذا كان رمز المسار موجوداً)
                let existingStudent = null;
                if (nationalId) {
                  existingStudent = await dbManager.getStudentByNationalId(nationalId);
                }

                if (existingStudent) {
                  await dbManager.updateStudent(existingStudent.id, student);
                  updatedCount++;
                  console.log(`🔄 تحديث: ${firstName} ${lastName} (${nationalId || 'بدون رمز'})`);
                  allErrors.push(`🔄 تم تحديث: ${firstName} ${lastName}${nationalId ? ' - ' + nationalId : ''}`);
                } else {
                  await dbManager.addStudent(student);
                  successCount++;
                  console.log(`✅ إضافة: ${firstName} ${lastName} (${nationalId || 'بدون رمز'}) - ر.ت: ${finalStudentId}`);
                  allErrors.push(`✅ تم إضافة: ${firstName} ${lastName}${nationalId ? ' - ' + nationalId : ''}`);
                }
                ...
                console.error(`❌ خطأ في الصف ${rowIndex + 1}:`, error);
              }
            }

            // ✅ ملخص بعد معالجة كل ورقة
            console.log(`\n📝 ملخص ورقة ${sheetName}:`);
            console.log(`  - إجمالي الصفوف المتوقعة: ${totalRows}`);
            console.log(`  - تم معالجتها بنجاح: ${successCount + updatedCount}`);
      ...
      // ✅ ملخص نهائي شامل
      console.log(`\n${'='.repeat(60)}`);
      console.log(`📊 ملخص الاستيراد الشامل`);
      console.log(`${'='.repeat(60)}`);
      console.log(`📁 ملفات معالجة: ${processedFiles}`);
      console.log(`📚 مستويات: ${uniqueLevels.size}`);
      console.log(`📖 أقسام: ${uniqueSections.size}`);
      console.log(`👥 تلاميذ جدد: ${successCount}`);
      console.log(`🔄 تلاميذ محدثين: ${updatedCount}`);
      console.log(`✅ إجمالي النجاح: ${successCount + updatedCount}`);
      console.log(`👨 ذكور: ${maleCount}`);
      console.log(`👩 إناث: ${femaleCount}`);
      console.log(`❌ أخطاء: ${errorCount}`);
      console.log(`${'='.repeat(60)}\n`);
